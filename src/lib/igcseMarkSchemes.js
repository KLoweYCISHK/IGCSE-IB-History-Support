// IGCSE question types and the generic mark-scheme level descriptors.
// Generic levels are pre-filled when an admin creates a question; the admin
// then edits each level to add the specific knowledge / examples for that question.
...
const TEN_MARK = [
  ...
  ['2', 'Identifies or describes valid points; addresses the question but does not explain. One Level 2 mark for each identification/description.', '2–3'],
  ['1', 'Writes about the topic but does not address the question.', '1'],
  ['0', 'No creditable response.', '0'],
];

const LEVELS = {
  '4_marker': SIX_MARK, // generic level comments stay the same; admin edits specifics
  '6_marker': SIX_MARK,
  '10_marker': TEN_MARK,
};
